...
function montsharkParser(interface) {
	...
	setInterval(function() {
		...
		function getAuth(sec) {
			if(sec == 2) {return "WPA2 PSK";}
			if(sec == 1) {return "WPA2 MGT";}
			if(sec == "") {return "Open";}
			return sec;
		}
		...
		sock.on('die', function(code) {
			console.log("Killing MonitorTshark with code: " + code);
			die = true;
			if(child){child.kill(code);}
		});
