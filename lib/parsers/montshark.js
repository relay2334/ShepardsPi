...
function montsharkParser() {
	var ssids = {};	// Transport array for maintaining protocol counts.
	...
	setInterval(function() {
		...
		// The next thing we need to do is copy the ssids array and then
	...
	function run() {
		...
		var child = spawn('tshark -f "type mgt subtype beacon" -T psml -PS -l -i ' + config.Monitoring.interface + 
						' -I', [], {shell: '/bin/bash'}
		);
