...
var interfaceManager = require("./interfaceManager").parser //Bash tool to manipulate interface mode
var config = require("config");
    ...
    socket.on('web', function (data) {
      ...
      activeInterfaces.forEach(function(i) {
        ...
        if (i.interface == data[0]) {
          ...
          i.socket.emit('die', 1);
          if(i.parser == "interfaceManager"){
            interfaceManager(data[0], "Managed");
          }
        }
      ...
      if (data[4]) {
        interfaceManager(data[0], "Monitor");
        montshark(data[0]);
      }
